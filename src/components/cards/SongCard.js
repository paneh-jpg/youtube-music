export function SongCard({
  ...
  return `
    <div data-slug="${slug}" data-id="${id}"
      class="js-song group flex items-center gap-4 lg:gap-5 cursor-pointer">

      <!-- Thumbnail-->
      <div class="relative w-14 h-14 lg:w-16 lg:h-16 shrink-0 rounded-md overflow-hidden">
        ...
        <!-- Play -->
        <button class="absolute inset flex items-center justify-center opacity-0 scale-95 group-hover:opacity-100 group-hover:scale-100 transition" >
      ...
      <div class="min-w-0">
        <!-- Song name -->
        <h4 class="text-white text-sm lg:text-[15px] font-semibold leading-snug truncate" title="${name}" >
          ${name}
        </h4>

        <!-- Meta -->
        <p class="text-white/60 text-xs lg:text-sm mt-1 truncate">
